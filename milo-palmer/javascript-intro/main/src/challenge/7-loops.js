...
const countShortNames = (array) => {
  let numberOfShortNames = 0
  for (const name of array) {
    if (name.length < 5) {
      numberOfShortNames++
    }
  }
  return numberOfShortNames
}
...
//loop through array only acting on every even number, When only % equal to zero

const everySecondItem = (array) => {
  let resultsArray = []
