...
function createCells(row,col,fn) {
  let cells = [];

  for (let i = 0; i < row; i++) {
    for (let j = 0; j < col; j++) {
      const cell = new Cell(i, j,fn);
      cells = [...cells, cell];
    }
  }
  return cells
...
function createBoard(row,col,fn) {
  const board = {
    cells:createCells(row,col,fn)
  }
  return board
}

const board=createBoard(ROW, COLUMN,getRandomNumber())
console.log(board);
