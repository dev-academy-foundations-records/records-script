...
function displayTransactionHistory() {
    const historyList=document.querySelectorAll('li')
    for (let i = 0; i < historyOrders.length;i++){
              historyList[i].innerHTML=historyOrders[i]
    }
}
...
function checkOrderFulfillment() {
    for (let i = 0; i < customer.order.length; i++) {
        ...
        if (products[productName].stock <= 0) {
            alert(`Sorry, We are out of ${productName} and your order has been cancelled`);
            customer.order = []
            displayCustomerOrder()
            return false
        } 
