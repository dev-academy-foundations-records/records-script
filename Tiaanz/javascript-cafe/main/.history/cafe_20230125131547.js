...
function displayTransactionHistory() {
  ...
  for (let i = 0; i < historyOrders.length; i++) {
    ...
    historyList.insertBefore(refundBtn, historyList.children[0]);
  }
}
...
/**************** TRANSACTION HISTORY AND REFUND ******************/
...
function displayTransactionHistory() {
  ...
  for (let i = 0; i < historyOrders.length; i++) {
    ...
    refundBtn.addEventListener("click", () => {
      const orderString = historyList.innerHTML.slice(
        0,
        historyList.innerHTML.length - 23
      );
      const orderArray = orderString.split(",");
      let refundTotal = 0;
      for (let x of orderArray) {
        refundTotal += products[x].price;
      }
      const r = confirm(
        `Are you sure to refund the order of ${orderString} with $${refundTotal}?`
      );
      if (r) {
        cash -= refundTotal;
        displayCash();
         const newHistoryOrders= historyOrders.filter(item => item !== orderArray)
          console.log(newHistoryOrders);
...
function refundOrder() {
  const orderHistory = document.querySelectorAll("li");
...
// console.log(refundButtons);
// for (let x of refundButtons){
