...
console.log(gradebook.studentName, gradebook.testScores)


// 5. Complete the average function that returns the average of a given array of numbers.
function average(arrSum) {
  let total = 0
  arrSum.forEach(num => {
      total += num
  });
  return total / arrSum.length
}
...
function getAverage(studentName) {
  let getAverage = average(gradebook[studentName].testScores)
  return getAverage
}
