...
//printFruit(['apple', 'orange', 'banana', 'pineapple'])
...
let arr = ['blast off', 'one', 'two', 'three']
let arrRev = arr.reverse()
function printCountdown(arr) {
  for (let i = 0; i < arr.length; i++) {
    let arrRevthing = arrRev[i]
    console.log(arrRevthing + '!')
  }
}
...
// 4. Define a function named `countShortNames` that takes an array of strings and returns a count of all the strings with 
// a length less than 5.
// For example: countShortNames(['Sally', 'John', 'James', 'Alice', 'Sue']) would return a count of 2 for 'John' and 'Sue', 
//as their names are less than 5 characters in length.
...
let arr = ['Sally', 'John', 'James', 'Alice', 'Sue']
let str = count[]
function countShortNames(arr){
  for (i = 0; i < str.arr.length; i++){
      return count
  } 
  for(let str of arr){
