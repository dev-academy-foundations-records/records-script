...
function countShortNames(arrayOfStrings) {
  let count = 0
  for (name of arrayOfStrings) {
    //can add const name
    if (name.length < 5) {
      count++
    }
...
/*function everySecondItem(arrayOfItems) {
	let newArray = []
	  arrayOfItems.forEach(element, index) {  //=> {}         item or element
	if (index % 2 ! = 0) {   Would it help to write out a regular loop to compare Or if (index % 2 == 1)
