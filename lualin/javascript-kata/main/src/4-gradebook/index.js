...
// Adds property to gradebook object with
...
for (let student in gradebook) {
  ...
  gradebook[student].testScores = scores[studentIdx]
}
...
function addScore(studentName, score) {
  const studentsScore = gradebook[studentName].testScores
...
function average(arr) {
  let averageNum = 0
  //Checks if all elements in given array are numbers
  const isAllNumbers = (arr) =>
    arr.every((element) => typeof element === 'number')
  // If all elements are numbers in the array, pass to next line otherwise return else string
  if (isAllNumbers(arr) == true) {
    let sumArray = 0
    // Sums up all numbers in the array
    for (let i = 0; i < arr.length; i++) {
      sumArray += arr[i]
    }
    // Gets average number by dividing total sumArray by the length of the array
    averageNum = sumArray / arr.length
    return averageNum
  } else {
    return 'Array with numbers only is required'
  }
}
...
function getAverage(studentName) {
  // Gets testScores array of given studentName from the gradebook and assign it to the averageArray
  let averageArray = gradebook[studentName].testScores
...
function enterScores(gradebook, students, scores) {
  //Until the iterator reaches before the length of students array, it creates properties of object
  for (let i = 0; i < students.length; i++) {
    gradebook[students[i]] = {} // Creates properties with key that has each element in students array and each key has an empty object value
    gradebook[students[i]].testScores = scores[i] // Creates key of testScores in the inner object that has value of scores
  }
  return gradebook
}
