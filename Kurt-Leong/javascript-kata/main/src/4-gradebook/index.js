...
// for (const key in gradebook) {
//   gradebook[key] = { testScores: scores[students.indexOf(key)] }
// }
...
function getAverage(studentName) {
  let studentScore = []
  studentScore = gradebook[studentName].testScores
  return average(studentScore)
}
...
const student = ['Joseph', 'Naomi', 'Yosan', 'David']
const score = [
  //Joseph's score
  [80, 70, 70, 100],
  // Naomi's score
...
function enterScores(gradebook, students, scores) {
  ...
  let students = []
  let scores = []
  gradebook[students].testScores = { scores }
}
