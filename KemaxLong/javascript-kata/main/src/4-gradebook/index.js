...
//   let score = scores[students.indexOf(student)]

//   let obj = gradebook[student]
//   // Add a new property with a key of testScores
//   obj.testScores = score
// })

for (let i = 0; i < students.length; i++) {
  let score = scores[i]
  let obj = gradebook[students[i]] //objects cannot loop
  obj.testScores = score
}
...
function addScore(studentName, score) {
  let name = gradebook[studentName]
  let newScore = name.testScores.push(score)
}
...
function getAverage(studentName) {
  let scoreAarry = gradebook[studentName].testScores
  let aVER = average(scoreAarry)
  return aVER
}
