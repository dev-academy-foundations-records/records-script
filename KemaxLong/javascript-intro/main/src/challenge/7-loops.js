...
const { buildFunctionResult } = require('../lib')
...
//i=4 0 = i=0+3 =3   i=1 i'=1+(1) i=2 i'= 2+(0)

function printCountdown(p) {
  for (let i = 0; i < p.length; i++) {
    console.log(p[p.length - 1 - i] + '!')
  }
}
...
function everySecondItem(k) {
  let g = []
  k.array.forEach((element, i) => {
    if (index % 2 == 1) {
      g.push(element)
    }
  })
