...
function superFizzbuzz(arrayParameter) {  
  ...
  fizzbuzzArray = [] // create an empty array to return

  arrayParameter.forEach(element => { // cycle through each element in arrayParameter
    output = fizzbuzz(element) //perform the fizzbuzz function on each element
    fizzbuzzArray.push(output) // add the result from the fizzbuzz function to the new array 
  });
  
  return fizzbuzzArray //return the new array
}
