...
// use index to loop through each student and their testscores
...
function addScore(studentName, score) {
  let scores = gradebook[studentName].testScores // navigate to correct students scores
...
function average(arr) {
  ...
  let total = 0             // add up all the scores
  arr.forEach(element => {
    total += element
    
  })
  
  return total / arr.length // divide by number of scores to find average
...
function getAverage(studentName) {
  ...
  let studentScores = gradebook[studentName].testScores
