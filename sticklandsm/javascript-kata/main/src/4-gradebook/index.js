...
for (let i = 0; i < scores.length; i++) {
  //I have to access the stuff in the object with a for loop, but you can't access objects by their numbered position
  //so you have to turn the gradebook into an array and grab the name that way so [Object.keys(gradebook)[i]] represets that.
...
function average(arr) {
  return arr.reduce((sum, currentScore) => sum + currentScore) / arr.length
}
