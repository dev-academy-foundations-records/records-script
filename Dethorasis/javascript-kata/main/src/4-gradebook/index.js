...
for (let i = 0; i < students.length; i++) {
  ...
  gradebook[currentStudent] = { newObject: (theScore = {}) }
  gradebook[currentStudent].testScores = scores[i]
}
...
function average(arr) {
  const average = arr.reduce((a, b) => a + b, 0) / arr.length //Finds the average using the reduce method
  return average
}
...
function getAverage(studentName) {
  gradebook[studentName] = average(gradebook[studentName].testScores)
