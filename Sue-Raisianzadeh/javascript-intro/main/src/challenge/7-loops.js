...
//**** The function "printResult" would be called 4 TimeRanges. as there are 4 fruits

// Uncomment the following line and assign your answer to the variable `numberPrintResultCalls`.

 const numberPrintResultCalls = 4 times
...
logNames(['Sally', 'Nikita', 'Arnav', 'Akira'])
logNames(['Arnav', 'Akira'])
...
function printCountdown(array) {
  for (let i = array.length-1; i >= 0; i--) {
    console.log(array[i]+"!")
  }
  }
  printCountdown(['blast off', 'one', 'two', 'three']) 
...
// Hint #1: for reference see: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/for...of
 function countShortNames(array) {
   for (let i = 0; i < array.length; i++) {
      if (array[i].length<5){
         console.log(array[i])
    }
 ...
 countShortNames(['Sally', 'John', 'James', 'Alice', 'Sue'])
...
function everySecondItem(array) {
 const newArray = [];
  array.forEach ((element, index) => {
   ...
   everySecondItem([1, 'test', 'something', 23, 42, {greeting: 'hi'}])
